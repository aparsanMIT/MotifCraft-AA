...
Examples:
  ...
  python boltzdesign_generalized.py --target_name 5zmc --target_type dna --pdb_target_ids C,D --target_mols SAM --binder_id A
    ...
    parser.add_argument(
        "--ccd_path", type=str, default="~/.boltz/ccd.pkl", help="Path to CCD file"
    )
    parser.add_argument(
        "--alphafold_dir", type=str, default="~/alphafold3", help="AlphaFold directory"
    )
    parser.add_argument(
        "--af3_docker_name", type=str, default="alphafold3", help="Docker name"
    )
    parser.add_argument(
...
def run_rosetta_step(
    args, ligandmpnn_dir, af_output_dir, af_output_apo_dir, af_pdb_dir, af_pdb_dir_apo
):
...
def get_target_ids(args):
    ...
    target_ids = (
        args.pdb_target_ids if args.input_type == "pdb" else args.custom_target_ids
    )

    if (args.contact_residues or args.modifications) and not target_ids:
        input_type = "PDB" if args.input_type == "pdb" else "Custom"
        raise ValueError(
            f"{input_type} target IDs must be provided when using contacts or modifications"
        )
        sys.exit(1)
...
def initialize_pipeline(args):
    ...
    config_obj = YamlConfig(
        main_dir=f"{work_dir}/inputs/{args.target_type}_{args.target_name}_{args.suffix}"
    )
...
def generate_yaml_config(args, config_obj):
    ...
    if args.input_type == "pdb":
        ...
        target_mols = (
            [str(x.strip()) for x in args.target_mols.split(",")]
            if args.target_mols
            else None
        )
        if args.pdb_path:
        ...
        target_inputs = (
            [str(x.strip()) for x in args.custom_target_input.split(",")]
            if args.custom_target_input
            else []
        )
