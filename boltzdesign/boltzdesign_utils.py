...
def get_boltz_model(
    checkpoint: Optional[str] = None, predict_args=None, device: Optional[str] = None
) -> Boltz1:
...
def boltz_hallucination(
    ...
    # if not pre_run:
    #     breakpoint()
    #     output = _run_model(boltz_model, batch, predict_args)
    #     alphabet = list("XXARNDCQEGHILKMFPSTWYV-")
    #     best_seq = "".join(
    #         [
    #             list("XXARNDCQEGHILKMFPSTWYV-")[i]
    #             for i in torch.argmax(
    #                 batch["res_type"][
    #                     batch["entity_id"] == chain_to_number[binder_chain], :
    #                 ],
    ...
    for step in range(semi_greedy_steps):
        ...
        if best_iptm > prev_iptm:
            ...
            for seq_data in [data, data_apo]:
                seq_data["sequences"][chain_to_number[binder_chain]]["protein"][
                    "sequence"
                ] = best_seq
            print(
                f"Step {step}, Epoch {best_id}, Update sequence, iptm {best_iptm}, previous iptm {prev_iptm}"
            )
...
def run_boltz_design(
    ...
    if config is None:
        config = {
            ...
            "pocket_conditioning": False,
            "msa_max_seqs": 4096,
            "length_min": 100,  # 95, # LENGTH IS SET HERE
            "length_max": 100,  #
            "helix_loss_min": -0.6,
            "helix_loss_max": -0.2,
            "optimizer_type": "SGD",
            ...
            for itr in range(design_samples):
                ...
                if save_trajectory:
                    ...
                    for i in range(len(traj_coords_list)):
                        current_coords = traj_coords_list[i][
                ...
                print("-" * 100)
                print(
                    f"Holo Protein PLDDT: {output['plddt'][:config['length']].mean():.3f}"
                )
                print(
                    f"Apo Protein PLDDT: {output_apo['plddt'][:config['length']].mean():.3f}"
                )
                print("-" * 100)
                print(
                    f"Holo Complex PLDDT: {float(output['complex_plddt'].detach().cpu().numpy()):.3f}"
                )
                print(
                    f"Apo Complex PLDDT: {float(output_apo['complex_plddt'].detach().cpu().numpy()):.3f}"
                )
                print("-" * 100)
                ...
                try:
                    ...
                    if loss_dir:
                        plt.savefig(
                            os.path.join(
                                loss_dir,
                                f'{target_binder_input}_loss_history_itr{itr + 1}_length{config["length"]}.png',
                            ),
                            facecolor="#1C1C1C",
                            edgecolor="none",
                            bbox_inches="tight",
                    ...
                    distogram_ani, sequence_ani = visualize_training_history(
                        ...
                        config["length"],
                        binder_chain=config["binder_chain"],
                        save_dir=animation_save_dir,
                        save_filename=f"{target_binder_input}_itr{itr + 1}_length{config['length']}",
                    )
                    if show_animation:
                        display(
                            HTML(
                                f"<div style='display:flex;gap:10px'><div style='flex:0.4'>{distogram_ani.to_jshtml()}</div><div style='flex:0.6'>{sequence_ani.to_jshtml()}</div></div>"
                            )
                        )
                ...
                result_yaml = os.path.join(
                    results_yaml_dir,
                    f'{target_binder_input}_results_itr{itr + 1}_length{config["length"]}.yaml',
                )
                ...
                best_sequence = "".join(
                    [
                        alphabet[i]
                        for i in np.argmax(
                            best_batch_cpu["res_type"][
                                best_batch_cpu["entity_id"]
                                == chain_to_number[config["binder_chain"]],
                                :,
                            ],
                            axis=-1,
                        )
                ...
                for seq in data["sequences"]:
                    if (
                        "protein" in seq
                        and "msa" in seq["protein"]
                        and seq["protein"]["msa"]
                    ):
                        seq["protein"]["msa"] = seq["protein"]["msa"].replace(
                            ".npz", ".a3m"
                        )
                ...
                data_apo["sequences"] = [
                    data_apo["sequences"][chain_to_number[config["binder_chain"]]]
                ]
                ...
                if redo_boltz_predict:
                    ...
                    save_confidence_scores(
                        ...
                        output,
                        best_structure,
                        f"{target_binder_input}_results_itr{itr + 1}_length{config['length']}",
                        0,
                    )
                    save_confidence_scores(
                        results_final_dir_apo,
